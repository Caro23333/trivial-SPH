...
vca, vcb, vcc = int((a - radius) // (2 * h) + 1), int((b - radius) // (2 * h) + 1), int((c - radius) // (2 * h) + 1)
vcd = 100
...
def setVoxel():
    ...
    ti.loop_config(serialize = True)
    for j, k in ti.ndrange(length, length):
        for t in range(2):
            i = t * length
            voxelPos = getVoxel(rigidParticles[i, j, k])
            voxelAdd(tm.ivec3(i, j, k), voxelPos, 1)
    for i, k in ti.ndrange(length, length):
        for t in range(2):
            j = t * length
            voxelPos = getVoxel(rigidParticles[i, j, k])
            voxelAdd(tm.ivec3(i, j, k), voxelPos, 1)
    for i, j in ti.ndrange(length, length):
        for t in range(2):
            k = t * length
            voxelPos = getVoxel(rigidParticles[i, j, k])
            voxelAdd(tm.ivec3(i, j, k), voxelPos, 1)
...
@ti.func
def pressureKernel(p: tm.ivec3) -> tm.vec3:
...
def couplingPressureKernel(p: tm.ivec3, q: tm.ivec3) -> tm.vec3:
    ...
    r = posr - posf
    rn = r.norm()
    return volumeCorrection(q, density0) * (p1 / (den * den)) * ((h - rn) ** r) * r / rn
...
def couplingViscosityKernel(p: tm.ivec3, q: tm.ivec3) -> tm.vec3:
    ...
    pi = -nu * min(tm.dot(v2 - v1, r), 0) / (rn * rn + eps * h * h)
    return -m * volumeCorrection(q, density0) * pi * \
           (-1.5 * r * r / (h ** 3) + 2 * r / (h * h) - h / (2 * r * r)) * r / rn * \
           15 / (2 * tm.pi * h ** 3)
...
def rigidParticleCollisionHandler(pos: tm.vec3, normal: tm.vec3, nowV: tm.vec3, cnt: ti.i32):
    ...
    # rigidPosition[None] += 2 * eps * normal
...
def rigidCollisionHandler():
    ...
    for p in range(6):
        ...
        for i, j, k in ti.ndrange(length, length, length):
            if 0 < i < length - 1 or 0 < j < length - 1 or 0 < k < length - 1:
                continue
        ...
        for i, j, k in ti.ndrange(length, length, length):
            ...
            pos = rigidParticles[i, j, k] + nowV * timeStep
            if pos[coordinateList[p]] * flag < boundaryList[p] * flag:
                rigidParticleCollisionHandler(pos, normalList[p], nowV, cnt)

@ti.func
def updateRigidKinematics(idx: tm.ivec3, force: tm.vec3):
    pos = rigidParticles[idx.x, idx.y, idx.z]
    rigidF[None] += force
    rigidP[None] += tm.cross(pos - rigidPosition[None], force)
...
def update():
    ...
    for i, j, k in ti.ndrange(na, nb, nc):
        ...
        for p, q, r in ti.ndrange((max(0, voxelPos.x - 1), min(vca, voxelPos.x + 2)), 
            ...
            for z in range(voxelCnt[p, q, r]):
                ...
                w = (position[i, j, k] - position[idx.x, idx.y, idx.z]).norm()
                if w <= h and w > shift:
                    tp = pressureGradientKernel(tm.ivec3(i, j, k), voxel[p, q, r, z])
                    tmpPressure += tp
            ...
            #     w = (position[i, j, k] - rigidParticles[idx.x, idx.y, idx.z]).norm()
            #     if w <= h:
            #         tp = couplingPressureKernel(tm.ivec3(i, j, k), tm.ivec3(p, q, r))
            #         tmpPressure += tp
            #         updateRigidKinematics(idx, K_spiky * tp)
            #         tv = couplingViscosityKernel(tm.ivec3(i, j, k), tm.ivec3(p, q, r))
            #         tmpViscosity += tv
            #         updateRigidKinematics(idx, -tv)
        acc = gravity - B * K_spiky * tmpPressure + tmpViscosity
    ...
    rigidV[None] += rigidF[None] * timeStep
...
def calcRigidVol(idx: tm.ivec3):
    ...
    for p, q, r in ti.ndrange((max(0, voxelPos.x - 1), min(vca, voxelPos.x + 2)), 
        ...
        for z in range(rigidVoxelCnt[p, q, r]):
            ...
            if w <= h:
                rho += rigidMass * densityKernel(w)
    rigidVol[i, j, k] = rigidMass / rho

@ti.kernel
def initRigid():
    rigidPosition[None] = tm.vec3(a / 2, b / 2, c * 2 / 3)
    for i, j, k in ti.ndrange(length, length, length):
        rigidParticles[i, j, k] = rigidPosition[None] + tm.vec3(i - length / 2, j - length / 2, k - length / 2) * radius
    ...
    rigidP[None] = rigidJ * tm.vec3(1, 1, 1)
    rigidV[None] = (0, 0, 0)
    rigidT[None] = (0, 0, 0)
    rigidF[None] = gravity    
    K_poly6 = m * 315 / (64 * tm.pi * h9)
...
while frameCnt < 1500:
    ...
    # render particles
    scene.particles(groupedPosition, radius = radius * 0.55, per_vertex_color = groupedColor)
    scene.particles(rigidGroupedParticles, radius = radius * 0.55, per_vertex_color = rigidGroupedColor)
    # render boundaries
    # scene.lines()
    canvas.scene(scene)
